...
async function generateAiText(article) {
    // This prompt is general enough for news
    const prompt = `
        You are a content curator focused on general news, adhering to journalistic best practices (clarity, accuracy, conciseness).
        TASK: Analyze the news based ONLY on the provided title. Generate the following content:
        1.  **image_headline**: A very concise headline (5-7 words, suitable for an image overlay). Make this bold using markdown (**headline**).
        2.  **short_description**: A brief description (max 40 words, MUST include key subjects mentioned in the title, suitable for an image overlay).
        3.  **social_caption**: A social media post caption (max 100 words total) ready for platforms like Instagram/Twitter, structured precisely as follows:
            ...
            * Include a relevant general hashtag like #News or #CurrentEvents.
...
async function generateSimplePreviewImage(imageUrl, overlayTextString) {
    ...
    try {
        ...
        let dynamicFontSize = Math.round(targetWidth * 0.032); // Slightly larger % for news
        if (dynamicFontSize < 18) dynamicFontSize = 18; // Larger min size
        if (dynamicFontSize > 40) dynamicFontSize = 40; // Larger max size
        
        const overlayFontSize = dynamicFontSize;
        // Adjust multiplier for Serif fonts (tend to be slightly narrower than Arial Black)
        const overlayCharsPerLine = Math.round(targetWidth / (overlayFontSize * 0.60)); 
        const overlayLines = wrapText(overlayText, overlayCharsPerLine, 2); // Max 2 lines
        ...
        // --- Dynamic SVG Generation (News Style) ---
        const lineSpacing = 1.25; // Slightly tighter line spacing
        const padding = Math.round(overlayFontSize * 0.65); // Adjust padding
        ...
        // Slightly taller overlay minimum height
        ...
        const textStartY = padding + overlayFontSize * 0.9; // Adjust vertical alignment slightly
        ...
        const compositeTop = Math.round(targetHeight - overlayHeight - 15); // Position slightly higher from bottom
